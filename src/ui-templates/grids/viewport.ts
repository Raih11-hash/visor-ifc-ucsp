/**
 * grids/viewport.ts — Rejilla flotante sobre el viewport 3D.
 *
 * Barra lateral izquierda: medición (longitud/área) y corte por plano.
 * Barra inferior: herramientas de visibilidad y selección (ver
 * toolbars/viewer-toolbar.ts). Solo una herramienta a la vez: al activar
 * una se desactivan las demás.
 */
  ...
  const leftToolbarTemplate: BUI.StatefullComponent = (_: {}, update) => {
    ...
    return BUI.html`
      <bim-toolbar style="align-self: start;" vertical>
        <bim-toolbar-section>
          <bim-button @click=${onMeasurementsClick} ?active=${areMeasurementsEnabled} label="Medición" tooltip-title="Medición" icon=${appIcons.RULER}>
            <bim-context-menu>
              <bim-button ?active=${lengthMeasurer.enabled} label="Longitud" @click=${onLengthMeasurement}></bim-button>
              <bim-button ?active=${areaMeasurer.enabled} label="Área" @click=${onAreaMeasurement}></bim-button>
            </bim-context-menu>
          </bim-button>
          <bim-button ?active=${clipper.enabled} @click=${onModelSection} label="Corte" tooltip-title="Corte del modelo" icon=${appIcons.CLIPPING}></bim-button> 
        </bim-toolbar-section>
      </bim-toolbar>
    `;
  };
